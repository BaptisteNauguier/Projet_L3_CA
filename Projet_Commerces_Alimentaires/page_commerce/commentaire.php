...
if(isset($_GET['status']) && $_GET['status'] == "add_commentaire" && $commentaire != ""){
		add_commentaire($_GET['id_ui'], $_GET['id_commerce'], $_GET['commentaire']);
		header('Location: page_commerce.php?id_commerce='.$_GET['id_commerce'].'');
		exit;
}else{
	header('Location: page_commerce.php?id_commerce='.$_GET['id_commerce'].''); 
}
...
function add_commentaire ($id_UI, $id_commerce, $commentaire)
{	
	$bdd = getBD();	
	$sql_commentaire = "INSERT INTO forum_ca (id_UI, id_commerce, avis) VALUES (?, ?, ?)"; 	
	$stmt_commentaire = $bdd->prepare($sql_commentaire);	
	$stmt_commentaire->execute([$id_UI, $id_commerce, $commentaire]) or die(print_r($stmt_commentaire->errorInfo(), true));
}
